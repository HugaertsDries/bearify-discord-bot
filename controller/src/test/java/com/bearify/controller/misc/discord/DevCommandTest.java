...
class DevCommandTest {
    ...
    void playbackPreviewPostsNamedPresetThroughSharedAnnouncer() {
        ...
        assertThat(broadcast.containers().get(0).accentColor()).isEqualTo(0xA84C28);
        assertThat(broadcast.containers().get(3).children())
                .extracting(child -> ((TextBlock) child).text())
                .anyMatch(text -> text.contains("**Up Next**"))
                .anyMatch(text -> text.contains("1. Hotel California"));
